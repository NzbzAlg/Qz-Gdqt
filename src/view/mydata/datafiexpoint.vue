  ...
  methods: {
    ...
    terminal() {
      if (this.selectedOptions.length === 1) {
        ...
        this.$http
          .get(`pc/group/findDevicesByGroup`, {
            params: {
    ...
    adjunction() {
      ...
      this.$http
        .post(`pc/fixed/updateFixedDevice`, info)
        .then(res => {
          var { code, data } = res.data;
          if (code === 1000) {
            this.$message({
            ...
            this.reload();
            this.$jsonp(
              `http://api.map.baidu.com/geocoder/v2/?ak=1IGwblSXzAV0yxzCq0ZGdYoixoreCQwS&callback=renderReverse&location=${this.lat},${this.lng}&output=json&pois=1 `
            )
              .then(res => {
                // var {code,data}=res.data
    ...
    handleDelete(index, rows, row) {
      ...
      this.$http
        .post(`pc/fixed/delFixedDevice`, info)
        .then(res => {
          var { code, data } = res.data;
          console.log(data);
          if (code === 1000) {
    ...
    edit() {
      ...
      this.$http
        .get(`pc/fixed/getFixedPriceBymerchantId`)
        .then(res => {
          var { code, data } = res.data;
          if (code === 1000) {
