  ...
  <div>
    ...
    <el-row>
      <div
        class="daochu"
        @click="exportc"
        style="display: inline;float:left;margin-left:20px;width:100px;height:45px;line-height:45px;background:#d9b4fa;border-radius: 5px;color:#fff;cursor: pointer;"
      >导出图表</div>
      <div
        class="daochu"
        @click="exportcarr"
...
const cityOptions1 = [
  { value: 5, label: "45岁以上" },
  { value: 6, label: "35-44岁" },
  { value: 7, label: "25-34岁" },
  { value: 8, label: "18-24岁" },
  { value: 9, label: "18岁以下" }
];
const cityOptions2 = [
  { value: 6, label: "高中以上" },
...
const cityOptions3 = [
  ...
  { value: 4, label: "3-5k" },
  { value: 5, label: "5-10k" },
  { value: 6, label: "10-20k" },
  { value: 7, label: "20k以下" }
];
const cityOptions4 = [
  { value: 2, label: "无未成年子女" },
  { value: 3, label: "0-3岁" },
  { value: 4, label: "4-6岁" },
  { value: 5, label: "7-12岁" },
  { value: 6, label: "13-17岁" }
];
const cityOptions5 = [
  { value: 1, label: "工作区域" },
  { value: 2, label: "居住区域" },
...
export default {
  ...
  methods: {
    ...
    getList() {
      let id = this.id;
      this.$http
        .post(`pc/fixedPortrait/selectCustomerAddr`, {
          taskId: id,
          aireType: 1
        })
        .then(res => {
      ...
      this.$http
        .post(`pc/fixedPortrait/selectCustomer`, { taskId: id, aireType: 1 })
        .then(res => {
          var { code, data } = res.data;
          if (code === 1000) {
            this.drawLine(data);
    ...
    dianji3() {
      ...
      // console.log(info)
      this.$http
        .post(`pc/fixedPortrait/selectCustomerAddr`, info)
        .then(res => {
          var { code, data } = res.data;
          if (code === 1000) {
            console.log(data);
      ...
      this.$http
        .post(`pc/fixedPortrait/selectCustomer`, info)
        .then(res => {
          var { code, data } = res.data;
          if (code === 1000) {
            this.drawLine(data);
    ...
    exportc() {
      ...
      // console.log(info)
      this.$http
        .post(`pc/fixedPortrait/exportCustomer`, info)
        .then(res => {
    ...
    exportcarr() {
      ...
      // console.log(info)
      this.$http
        .post(`pc/fixedPortrait/exportCustomerAddr`, info)
        .then(res => {
          let filePath = res.data.data.path;
          let fileName = res.data.data.fileName;
          window.location.href = `http://47.105.207.228:8874/pc/fixedPortrait/export/fixed?filePath=${filePath}&fileName=${fileName}`;
